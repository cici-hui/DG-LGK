    ...
    def __getitem__(self, index):
        ...
        while (1):
            ...
            if self.transform != None:
                image_nochange, image, label = self.transform(image_PIL, label_PIL)
            image_nochange = np.asarray(image_nochange, np.float32)
        ...
        image_nochange = image_nochange[:, :, ::-1]  # change to BGR
        image_nochange -= self.mean
        image_nochange = image_nochange.transpose((2, 0, 1)) / 128.0

        image = image[:, :, ::-1]  # change to BGR
        image -= 128  # self.mean
        image = image.transpose((2, 0, 1)) / 128.0

        return image_nochange.copy(), image.copy(), label_copy.copy(), boundary_gt
